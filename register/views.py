                ...
                if (("pdf" in request.FILES) and (request.FILES["pdf"])):
                    ...
                    with switch(tipoDocString) as s:
                        if s.case('Invoice', True): 
                            ...
                            nrodocemisor = cpe[tipoDocString]['cac:AccountingSupplierParty']['cac:Party']['cac:PartyIdentification']['cbc:ID']['#text']
                            nrodocreceptor = cpe[tipoDocString]['cac:AccountingCustomerParty']['cac:Party']['cac:PartyIdentification']['cbc:ID']['#text']
                            tipodocreceptor = cpe[tipoDocString]['cac:AccountingCustomerParty']['cac:Party']['cac:PartyIdentification']['cbc:ID']['@schemeID']
                            razonsocialreceptor = cpe[tipoDocString]['cac:AccountingCustomerParty']['cac:Party']['cac:PartyLegalEntity']['cbc:RegistrationName']
                        if s.case('CreditNote', True): 
                            ...
                            serierefCpe = cpe[tipoDocString]['cac:BillingReference']['cac:InvoiceDocumentReference']['cbc:ID'].split('-')[0] 
                            numerorefCpe = cpe[tipoDocString]['cac:BillingReference']['cac:InvoiceDocumentReference']['cbc:ID'].split('-')[1] 
                            tipocperef = cpe[tipoDocString]['cac:BillingReference']['cac:InvoiceDocumentReference']['cbc:DocumentTypeCode']['#text'] 
                            tipocperefId = TipoCPEModel.objects.get(tipocpeCod = tipocperef).tipocpeId 
                        ...
                        if s.case('DespatchAdvice', True): 
                            ...
                            tipodocreceptor = cpe[tipoDocString]['cac:DeliveryCustomerParty']['cbc:CustomerAssignedAccountID']['@schemeID']
                            razonsocialreceptor = cpe[tipoDocString]['cac:DeliveryCustomerParty']['cac:Party']['cac:PartyLegalEntity']['cbc:RegistrationName']
                        ...
                        if s.case('Retention', True): 
                            ...
                            tipodocreceptor = cpe[tipoDocString]['cac:ReceiverParty']['cac:PartyIdentification']['cbc:ID']['@schemeID']
                            razonsocialreceptor = cpe[tipoDocString]['cac:ReceiverParty']['cac:PartyLegalEntity']['cbc:RegistrationName']
                    ...
                    emisorId = EmisorModel.objects.get(emisorNroDoc = nrodocemisor).emisorId  
                    
                    tipocpeId = TipoCPEModel.objects.get(tipocpeCod = tipocpe).tipocpeId
                    tipodocId = TipoDOCModel.objects.get(tipodocCod = tipodocreceptor).tipodocId
                    ...
                    dicReceptor.setdefault('receptorRazonSocial', razonsocialreceptor)


                    cpeVal = []
                    try:
                        cpeVal = self.get_queryset().filter(serieCpe = seriecpe, numeroCpe = numerocpe, emisorId = emisorId).first()
                    except:
                    ...
                    if not cpeVal:
                        receptor = []
                        try:
                            receptor = ReceptorModel.objects.get(receptorNroDoc = nrodocreceptor, tipodocId = tipodocId, receptorRazonSocial = razonsocialreceptor)
                        except:
                        ...
                        if not receptor:
                            # Crear Receptor
                            receptorCreado = ReceptorSerializer(data = dicReceptor)
                            if receptorCreado.is_valid(raise_exception=True):
                                receptorCreado.save()
                            receptor = ReceptorModel.objects.get(receptorNroDoc = nrodocreceptor, tipodocId = tipodocId)
                        ...
                        dicCpe.setdefault('fechapublicacionCpe', datetime.now()) 
            ...
            if (("xml" in request.FILES) and (request.FILES["xml"])):
                ...
                if (("pdf" in request.FILES) and (request.FILES["pdf"])):
                    ...
                    if not cpeVal:
                        ...
                        dicCpe.setdefault('fechaemisionresumen', fechaemisionresumen) 
                        dicCpe.setdefault('fechageneracionresumen', fechageneracionresumen) 
                        dicCpe.setdefault('tipocpeId', tipocpeId)
                        dicCpe.setdefault('emisorId', emisorId) 
                        dicCpe.setdefault('estadoId', estadoId)  
                        dicCpe.setdefault('fechapublicacionresumen', datetime.now())   
                        dicCpe.setdefault('usuId', 1) 
                            ...
                            if respuesta.is_valid(raise_exception=True):
                                ...
                                if(tipocpe == 'RC'):
                                    ...
                                    if isinstance(detalle, List):
                                        ...
                                        for item in detalle: 
                                            tipocpeId = TipoCPEModel.objects.get(tipocpeCod = item['cbc:DocumentTypeCode']).tipocpeId  
                                            dicCPE = dict()
                                            dicCPE.setdefault('resumenId', resumen.resumenId)
                                            dicCPE.setdefault('serierefCpe', item['cbc:ID'].split('-')[0])
                                            dicCPE.setdefault('numerorefCpe', item['cbc:ID'].split('-')[1])
                                            dicCPE.setdefault('tipocperefId', tipocpeId) 
                                            cpeCreado = CPEResumenSerializer(data = dicCPE)
                                            if cpeCreado.is_valid(raise_exception=True):
                                                cpeCreado.save()
                                    else:
                                        # El CPE solo tiene 1 detalle  
                                        tipocpeId = TipoCPEModel.objects.get(tipocpeCod = detalle['cbc:DocumentTypeCode']).tipocpeId  
                                        dicCPE = dict()
                                        dicCPE.setdefault('resumenId', resumen.resumenId)
                                        dicCPE.setdefault('serierefCpe', detalle['cbc:ID'].split('-')[0])
                                        dicCPE.setdefault('numerorefCpe', detalle['cbc:ID'].split('-')[1])
                                        dicCPE.setdefault('tipocperefId', tipocpeId)
                                ...
                                if(tipocpe == 'RA'):
                                    ...
                                    if isinstance(detalle, List):
                                        ...
                                        for item in detalle: 
                                            ...
                                            dicCPE = dict()
                                            dicCPE.setdefault('resumenId', resumen.resumenId) 
                                            dicCPE.setdefault('serierefCpe', item['sac:DocumentSerialID']) 
                                            dicCPE.setdefault('numerorefCpe', item['sac:DocumentNumberID']) 
                                            dicCPE.setdefault('tipocperefId', tipocpeId) 
                                            cpeCreado = CPEResumenSerializer(data = dicCPE)
                                            if cpeCreado.is_valid(raise_exception=True):
                                                cpeCreado.save()  
                                        ...
                                        dicCPE = dict()
                                        dicCPE.setdefault('resumenId', resumen.resumenId)
                                        dicCPE.setdefault('serierefCpe', detalle['sac:DocumentSerialID'])
                                        dicCPE.setdefault('numerorefCpe', detalle['sac:DocumentNumberID'])
                                        dicCPE.setdefault('tipocperefId', tipocpeId)
...
class ActualizaTicketView(CreateAPIView):
    ...
    def put(self, request): 
        try:
            ...
            emisorId = EmisorModel.objects.get(emisorNroDoc = emisorNroDoc).emisorId   
            tipocpeId = TipoCPEModel.objects.get(tipocpeCod = tipocpe).tipocpeId 

 
            cpeVal = []
            try:
                cpeVal = self.get_queryset().filter(ID = ID, emisorId = emisorId, tipocpeId = tipocpeId).first()
            except ValueError:
                return Response({
                    'status' : False,
                    'content' : None,
...
class ActualizacionResumenView(CreateAPIView):
    ...
    def put(self, request, ID):
        if (("cdr" in request.FILES) and (request.FILES["cdr"])):
            ...
            estadoBaja = '03'
            estadoBajaId = EstadoModel.objects.get(estadoCod = estadoBaja).estadoId
            ...
            cpeVal = []
            try:
                cpeVal = self.get_queryset().filter(ID = IDResumen, tipocpeId = tipocpeId, emisorId = emisorId).first()  
            except ValueError:
                return Response({
                    'status' : False,
                    'content' : None,
            ...
            try: 
                ...
                if respuesta.is_valid(raise_exception=True): 
                    ...
                    for item in CPEResumenModel.objects.filter(resumenId = cpeVal.resumenId): 
                        CPEModel.objects.filter(serieCpe = item.serierefCpe, numeroCpe = item.numerorefCpe, tipocpeId = item.tipocperefId, emisorId = emisorId).update(estadoId = estadoBajaId)
...
class CPEView(ListCreateAPIView):
    ...
    def get(self, request, rucEmisor,tipoDocReceptor,rucReceptor,tipoCpe,serieCpe,numeroCpe,fechaCpe,totalCpe): 
            ...
            receptorId = 0
            try:
                receptorId = ReceptorModel.objects.get(receptorNroDoc = rucReceptor, tipodocId = tipoDocReceptor).receptorId
            except:
                pass
            
            respuesta = self.get_queryset().filter(serieCpe = serieCpe, numeroCpe = numeroCpe, emisorId = emisorId, tipocpeId = tipoCpe, fechaCpe = fechaCpe, totalCpe = totalCpe).first()
...
class ValidarEmisorView(ListCreateAPIView):
    ...
    def get(self, request, rucEmisor, claveEmisor): 
        ...
        emisorId = 0
        try:
            emisorId = EmisorModel.objects.get(emisorNroDoc = rucEmisor, emisorClave = claveEmisor, estado = 1).emisorId
        except:
            pass
